	...
	if (result == -1) {
		...
		while (1) {
			...
			case 1:
				...
				if (isLittle == false) {
					taskCode = changeEndian(taskCode);
					requestID = changeEndian(requestID);
					numberToSend = changeEndian(numberToSend);
				}
				...
				if (isLittle == false) { numberToSend = changeEndian(numberToSend); }
			...
			case 2:
				...
				if (isLittle == false) { actualTimeFromServer = changeEndian(actualTimeFromServer); }
